        ...
        try:
            ...
            if serializer.is_valid():
                ...
                if self.send_invoice_email(order):
                    # Include the serialized order data in the response
                    serialized_order = OrderSerializer(order).data
                    return Response({'status': True, 'message': 'Order created successfully', 'order': serialized_order}, status=status.HTTP_201_CREATED)
                else:
...
def post(self, request):
    try:
        ...
        if serializer.is_valid():
            ...
            # Check if the user is associated with the order
            if self.send_invoice_email(order):
                # Get the serialized data of the created order
                serialized_order = OrderSerializer(order).data
